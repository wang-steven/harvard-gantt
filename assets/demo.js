...
ganttApp.controller("ganttController", ['$scope', '$http', '$location', function($scope, $http, $location) {
    ...
    $scope.addSamples2 = function() {
        $scope.loadData(getSampleData2().data);
    };
  ...
  //           // load data from sample
    ...
    $scope.triggerMaxHeight = function() {
        if ($scope.maxHeight === 0) {
            $scope.maxHeight = 320;//;(window.outerHeight - (document.getElementById('form-box').offsetTop + document.getElementById('form-box').offsetHeight)) - 150;
        } else {
            $scope.maxHeight = 0;
        }
    };
